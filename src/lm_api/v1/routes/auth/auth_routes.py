...
@router.post("/token", response_model=GetTokenResponse, summary="Obtenir un token d'accès", tags=["auth"])
async def login(
        username: str = Form(...),
        password: str = Form(...),
    ...
    """
    try:
        token_data = await auth.generate_token(username, password, timedelta(minutes=expires_in))
        return token_data
...
@router.post("/register", response_model=RegisterResponse, summary="Enregistrer un nouvel utilisateur", tags=["auth"])
async def register(
        auth_request: RegisterRequestBody,
        auth: APIAuth = Depends(get_auth)
):
    """
...
@router.get("/verify", response_model=VerifyResponse, summary="Vérifier le token d'accès", tags=["auth"])
async def verify(token: str = Depends(oauth2_scheme), auth: APIAuth = Depends(get_auth)):
    """
