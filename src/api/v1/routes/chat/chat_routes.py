...
async def completions(
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="Service indisponible ou modèle non trouvé")

    # Formatage des messages
    messages = text_generation.inference_utils.format_messages(chat_request.prompt, chat_request.input,
    ...
    await request.app.mongodb_client.insert_one("chat_completions_events", {
    ...
    if chat_request.stream:
        return StreamingResponse(
            text_generation.generate_stream_response(
                ...
                max_tokens=chat_request.max_tokens, top_p=chat_request.top_p, job_id=job_id
            ),
